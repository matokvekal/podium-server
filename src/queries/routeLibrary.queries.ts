// SQL for THE ROUTE LIBRARY — the standalone, reusable `routes` table.
//
//   /api/v1/routes/*                ->  this file
//   /api/v1/events/:eventId/route   ->  eventRoute.queries.ts
//
// The `event_routes` join table is NOT here; it belongs to the event-attached route and
// lives in eventRoute.queries.ts. This file reads track_points as TrackPoint objects — see
// the shape warning at the top of eventRoute.queries.ts before changing that.
//
...
export const ROUTE_OWNER_COLUMN = `
...
export function mapRouteWithOwner(row: RouteRow): RouteWithOwner {
